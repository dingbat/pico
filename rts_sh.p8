...
techs=0
t=0
npl=0]]aspl"dq,exp,vc,dmaps,units,rtiles,sel,prj,bldgs,nviz,ais,dms"res1,dms.d,pi,ptr,cf,selt,alert,ban,amx,amy,tot,loser,title=res.p1,{},split"1,2,3,4",us"0x9004,59,0,0,0,64,64,50"for i=2,4do ais[i]=p("boi=0",i)end p[[var=heal
qty=.00083]]p[[var=ren
r=0
g=0
b=6
...
gth=1
ez_adj=1]],wp or path(u,t.x,t.y),t,f2r[fm(tx,ty)],tx,ty)end local function gd(u,nxt_res,dr)local wayp if not dr then wayp,x,y=dpath(u,"d")dr=(not wayp or g(bldgs,x,y,{}).ply!=u.ply)and units[u.ply]end u.st=p([[t=drop
drop=1
in_bld=1
ez_adj=1]],dr and path(u,dr.x,dr.y)or wayp,dr or tu(x,y),nxt_res)end function ga(u,e)if e then u.st,u.disc,u.res=p([[t=atk
active=1
k=]]..e.k,path(u,e.x,e.y,0,u.typ.rng/10),e),u.disc or e.hu and u.bldg end end local function gf(u,f)f.farmer,u.st,u.res=u,p([[t=frm
