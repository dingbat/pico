...
return end aspl"bfog,afog,btns"for u in all(units)do if u.onscr or loser then if not loser and not g(viz,u.x8,u.y8)and u.disc then add(afog,u)elseif u.bldg or u.dead then du(u)else add(bfog,u)end end end foreach(bfog,du)local cf5,dt=cf\5,t()-hlt camera(cx,cy)foreach(prj,function(_ENV)sspr(typ.s+cf5%2*2,96,2,2,x,y)end)if loser then resbar()bnr(loser,split"defeat⁶x2....⁶x4⁶jdnᶜ1defeat⁶x2....,victory!⁶jdnᶜ1victory!"[loser],stat"54">56and"press ❎ for menu ⁴f⁶x1 "or"thx for playingᶜ8♥ ⁴f⁶x1 ",ban)return end pspl"0,5,13,13,13,13,6,2,6,6,13,13,13,0,5"draw_map"48"_pal,pal=pal,max foreach(afog,du)pal=_pal pal()fillp"23130.5"for x=cx\8,cx\8+16do for y=cy\8,cy\8+13do local i=x|y<<8local function b(a,col)color(col)camera(cx-x*8,cy-y*8)return a[i-1]and unl"-1,0,-1,7",a[i-256]and unl"0,-1,7,-1",a[i+256]and unl"0,8,7,8",a[i+1]and unl"8,0,8,7"end if not exp[i]then b(exp)elseif not viz[i]then b(viz,fm(x,y)>=128or 5)end end end camera(cx,cy)fillp(selx and rect(unpack(sb)),sel1 and sel1.rx and spr(64+cf5%3,sel1.rx-2,sel1.ry-5))if dt>.5then p"var=hlv"elseif hlv.f then circ(hlv.typ,hlv.x,min(hlv.f/dt,4),hlv.c)elseif mid(dt,.1,.25)!=dt and hlv.r then rect(unpack(hlv.r))end draw_menu()cp()if not hlv.p1 then circ(unpack(hlv))end if to_bld then camera(cx-mx8*8,cy-my8*8)pspl(bldable()or"8,8,8,8,8,8,8,8,8,8,8,8,8,8,8")if amy>=104then camera(4-amx,4-amy)else fillp"23130.5"rect(to_bld.fw,to_bld.h,unspl"-1,-1,3")fillp()end local _ENV=to_bld sspr(rt_x,rt_y,fw,h)pal()end camera(-amx,-amy)if dget"0">1then spr(hbtn and pset(unspl"-1,4,5")and 188or sel1 and sel1.hu and((to_bld or can_bld()or cr"1")and 190or can_gth()and 189or can_drop()and 191or can_atk()and(st.mnk and 185or 187))or 186)end end local function rt(_ENV)st=p[[t=rt
agg=1
idl=1]]end function can_pay(typ,_ENV)typ.reqs=reqs|typ.bq==reqs return r>=typ.r and g>=typ.g and b>=typ.b and(not typ.unit or pop<min(pl,99))and typ.reqs end local function pay(typ,dir,_ENV)r-=typ.r*dir g-=typ.g*dir b-=typ.b*dir if typ.unit then pop+=dir end end local function reg_bldg(b)local x,y=b.x8,b.y8 local function reg(xx,yy)s(bldgs,xx,yy,b.alive and b)if b.dead then s(exp,xx,yy,1,b.typ.f and y==yy and mset(xx,yy,69))s(dmap_st.d,xx,yy)elseif b.drop then s(dmap_st.d,xx,yy,{xx,yy})end end reg(x,y,b.h8 or reg(x,y-1),b.w8 or reg(x+1,y,b.h8 or reg(x+1,y-1)))if not b.ct and not b.farm then qdmaps"d"b.pres.reqs|=b.bmap end end function g(a,x,y,def)return a[x|y<<8]or def end function s(a,x,y,v)a[x|y<<8]=v end function hilite(v)hlt,hlv=t(),v end function int(r1,r2,p)return r1[1]-p<r2[3]and r1[3]+p>r2[1]and r1[2]-p<r2[4]and r1[4]+p>r2[2]end local function box(_ENV)r,x8,y8,dmgd,ai,ap,pres={x-w/2-1,y-h/2-1,x+w/2,y+h/2,8},x\8,y\8,hp<max_hp,gl.ais[ply],ply|9,gl.res[ply]k,hu=x8|y8<<8,not ai if not ct then hp+=typ.hp-max_hp max_hp=typ.hp end return _ENV end function tu(tx,ty)return box(p([[ply=0
ais=
hp=0
max_hp=0
ct=1
w=8
h=8]],nil,tx*8+4,ty*8+4))end function dist(dx,dy)local x,y=dx>>31,dy>>31local a0,b0=dx+x^^x,dy+y^^y return a0>b0 and a0*.9609+b0*.3984or b0*.9609+a0*.3984end local function surr(fn,x,y,n,na)local n,e=n or 1for dx=-n,n do for dy=-n,n do local xx,yy=x+dx,y+dy if min(xx,yy)>=0and xx<48and yy<32and(na or acc(xx,yy))then e=e or dx|dy!=0if fn then fn{xx,yy,d=dx&dy~=0and 1.4or 1,k=xx|yy<<8}end end end end return e end function afarm()local _ENV=hbld return farm and not exp and not farmer and not ct end function can_gth()local t=mget(mx8,my8)return(st.ant and(resqty[t]or afarm())or st.mnk==t)and g(exp,mx8,my8)and surr(nil,mx8,my8)end function can_atk()return sel1.atk and hunit and(not hunit.hu or st.mnk and hunit.dmgd and not hunit.bldg)and g(viz,mx8,my8,hunit.disc)end function can_bld()return hbld.hu and hbld.hp<hbld.typ.hp and(st.ant or hbld.web and st.sp)end function norm(it,nt,f)local dx,dy=it[1]-nt.x,it[2]-nt.y d,nt.dir=dist(dx,dy)+.0001,sgn(dx)nt.x+=dx*f/d nt.y+=dy*f/d return d<1end function acc(x,y,strict)local _ENV=g(bldgs,x,y)return not fget(mget(x,y),0)and(not _ENV or web and spdr or not strict and(ct or farm))end function bldable()return acc(mx8,my8,1)and(to_bld.h8 or acc(mx8,my8+1,1))and(to_bld.w8 or acc(mx8+1,my8,1)and acc(mx8+1,my8+1,1))end function dmg(typ,to)to.hp-=typ.atk*dmg_mult[typ.atk_typ..to.def]if to.st.idl or to.st.y then w(to)end to.conv+=typ.conv if to.ai and to.grp~="atk"then to.ai.safe=mvg(to.ai.p1,to.x,to.y,1)end if to.onscr then poke(13480,rnd"32",rnd"32")sfx(typ.sfx)alert=t()elseif to.hu and t()-alert>10then sfx"34"hilite{105+to.x/20.21,107+to.y/21.33,3,14}alert=hlt hlt+=2.5end end function can_drop()for u in all(sel)do if u.res then return hbld.hu and hbld.drop end end end function cr(t)if hbld.exp and st.ant then pres(ren,10,2)rect(unspl"8,0,18,8,4")return can_pay(ren,res1)or t end end function prod(_ENV,b,m)pay(b,1,pres)q=q or p("qty=0",b,b.t*m)q.qty+=1end function unit(t,_x,_y,_ply,_ct,_disc,_hp)tot+=1local _typ=typs[t]or t local _ENV=add(units,p([[dir=1
lp=1
sproff=0
cyc=0
fres=0
conv=0
alive=1
bop=101]],_typ[_ply],rnd"60"\1))foreach(gl.split"w8,h8,bldg,unit,farm,idx,qn,web,ant,mnk,w,h,atk,def,drop,sp,sg,bldrs,bmap,hpr,l",function(k)_ENV[k]=typ[k]end)max_hp=_ct and _ct>0and _ct*hpr or typ.hp id,x,y,ply,hp,ct,disc,prod=x,_x,_y,_ply,min(_hp or 9999,max_hp),max(_ct)>0and _ct,_disc==1,_typ.prod or{}rt(box(_ENV))return _ENV,bldg and reg_bldg(_ENV)end function start()npl,hq,cx,cy=res1.npl,units[1],unspl(stp[res1.pos],":")qdmaps"d"end function init()poke(24365,3)reload()music(unspl"0,0,7")menuitem(3,"⌂ save",save)menuitem(4,"∧ resign",function()hq.hp=0end)p[[var=res
r=20
g=10
b=20
pop=4
...
techs=0
t=0
npl=0]]aspl"dq,exp,vc,dmaps,units,rtiles,sel,prj,bldgs,nviz,ais,dmap_st"res1,dmap_st.d,pi,ptr,cf,selt,alert,ban,amx,amy,tot,loser,title=res.p1,{},split"1,2,3,4",unspl"0x9004,59,0,0,0,64,64,50"for i=2,4do ais[i]=p("boi=0",i)end p[[var=heal
qty=.00083]]p[[var=ren
r=0
g=0
b=6
...
ptx=51
pty=80]],archer,function(_ENV)los,rng=40,35end),nil,nil,p([[t=18
r=15
g=7
b=0
bq=0
...
ptx=69
pty=80]],castle,function(_ENV)los,rng=55,50end)}end p[[var=f2r
7=r
10=g
11=g
19=b
...
bldsp=1.25
bldsg=.9
bldbld=.1]]local function mv(u,x,y,agg)u.st=p([[t=mv
mv=1]],path(u,x,y,0))u.st.agg=agg end function mvg(units,x,y,agg,frc)local l=999foreach(units,function(_ENV)if frc or st.idl then mv(_ENV,x,y,agg)end l=min(typ.spd,l)end)foreach(units,function(_ENV)st.spd,grp=l,agg end)end local function gobld(u,b)if u.st.farm and b.farm then return end u.st,u.res=p([[t=bld
in_bld=1
ez_adj=1]],path(u,b.x,b.y),b)end local function gogth(u,tx,ty,wp)local t=tu(tx,ty)u.st=p([[t=gth
gth=1
ez_adj=1]],wp or path(u,t.x,t.y),t,f2r[fm(tx,ty)],tx,ty)end local function godrop(u,nxt_res,dr)local wayp if not dr then wayp,x,y=dpath(u,"d")dr=(not wayp or g(bldgs,x,y,{}).ply!=u.ply)and units[u.ply]end u.st=p([[t=drop
drop=1
in_bld=1
ez_adj=1]],dr and path(u,dr.x,dr.y)or wayp,dr or tu(x,y),nxt_res)end function goatk(u,e)if e then u.st,u.disc,u.res=p([[t=atk
active=1
k=]]..e.k,path(u,e.x,e.y,0,u.typ.rng/10),e),u.disc or e.hu and u.bldg end end local function gofarm(u,f)f.farmer,u.st,u.res=u,p([[t=frm
in_bld=1]],path(u,f.x+rs"-2,-1,0,1,2",f.y+rs"-2,-1,0,1,2"))u.st.farm=f end function tick(u)u.onscr,u.upd,wayp,ut=int(box(u).r,{cx,cy,cx+128,cy+104},0),u.id%z==upc,u.st.typ,u.typ if u.hp<=0and u.alive then tot-=1u.st,u.dead,u.farmer,u.alive=p"t=dead",ut.d del(sel,u,u.bldg and reg_bldg(u),u.onscr and sfx(ut.dsfx))if ut.l then local t=nr(u.x,u.y)mset(t[1],t[2],82+u.dir)dmap_st.r[t.k]=t qdmaps"r"elseif u.qn then npl-=1if npl==1or u==hq then loser,sel=min(u.ply,2),{}music"56"end elseif ut.drop and not u.ct then u.pres.pl-=ut.drop elseif u.unit then u.pres.pop-=1end end if u.dead then u.dead+=1del(u.dead==60and units,u)return end if wayp then if norm(wayp[1],u,u.st.spd or ut.spd)then deli(wayp,1)u.st.typ=wayp[1]and wayp end elseif u.st.mv then rt(u)elseif u.st.farm then u.st.active=1end local x,y,t,los,agg_d,agg_u=u.x,u.y,u.st.x,ut.los,9999,u.q and pr(u)if u.farm then local _ENV=u if farmer and farmer.st.farm!=u or exp then farmer=nil end end if t then if t.dead then u.st.agg=1,wayp or rt(u),u.ant and t.l and gogth(u,t.x8,t.y8)elseif int(t.r,u.r,-2)then u.st.active,u.st.typ=1elseif u.st.gth and not wayp then gogth(u,t.x8,t.y8)end if not wayp then u.dir=sgn(t.x-x)end end if u.st.active then _ENV[u.st.t](u)end if ut.hl and u.dmgd then u.hp+=heal[u.ply].qty end if int(u.r,{mx,my,mx,my},1)and(not hunit or hunit.hu)then hunit=u end if g(viz,u.x8,u.y8,u.disc)then if selx and int(u.r,sb,0)then if not u.hu then sele={u}elseif u.unit then selh=selh or{}add(selh,u)else selb={u}end end sset(109+x/20.21,72+y/21.33,u.ap)end
if(u.ct)return
if u.st.idl then
if(ut.l and t6)w(u)
if u.hu then if u.ant then if u.st.idl>10then idl=u end u.st.idl+=1elseif ut.idl and not u.q then idlm=u end end end if u.upd then if u.hu then local xo,yo,l=x%8\2,y%8\2,ceil(los/8)local k=xo|yo*16|los*256if not vc[k]then vc[k]={}for dx=-l,l do for dy=-l,l do add(dist(xo*2-dx*8-4,yo*2-dy*8-4)<los and vc[k],dx+dy*256)end end end foreach(vc[k],function(t)local k=u.k+t;(bldgs[k]or{}).disc,exp[k],nviz[k]=1,128,"v"end)end if u.st.agg and u.atk then for e in all(units)do if e.ap!=u.ap or u.mnk and e.dmgd and not e.bldg then local d=dist(x-e.x,y-e.y)if e.alive and d<=los then if e.bldg then d+=u.sg and e.bldg==1and-999or 999end if d<agg_d then agg_u,agg_d=e,d end end end end goatk(u,agg_u)end end if u.unit and not u.st.typ then local fr,v={{x,y}},{}for i,t in next,fr do x,y=unpack(t)local a=u.st.ez_adj or acc(x\8,y\8)if a or i==1then if a and not g(pos,x\4,y\4)then u.st.typ=i>1and{t}break end for nx=max(x-2),min(x+2,382),2do for ny=max(y-2),min(y+2,253),2do s(v,nx\2,ny\2,add(g(v,nx\2,ny\2,fr),{nx,ny}))end end end end s(pos,x\4,y\4,1)end end function du(u)local ut,r,w,h,stt,ihp,ux,uy=u.typ,u.res and u.res.typ or"_",u.w,u.h,u.st.typ and"mv"or u.st.t,u.max_hp/u.hp,unpack(u.r)local fw,sx,sy,ufps,fr,f,selc=ut.fw,ut[stt.."_x"]+resx[r]+u.sproff\8*8,ut[stt.."_y"]+resy[r],ut[stt.."_fps"],ut[stt.."_fr"],u.dead or(cf-u.id)%60,count(sel,u)==1and 9camera(cx-ux,cy-uy)if u.ct and u.alive then fillp"23130.5"rect(0,0,w,h,selc or 12)fillp()local r=u.ct/u.typ.ct line(fw-1,unspl"0,0,0,5")line(fw*r,0,14)sx+=r\-.5*fw if r<=.15then return end elseif ufps then sx+=f\ufps%fr*fw end pal{selc or u.pres.col,2,3,4,u.farm and selc or 5,unspl"6,7,8,9,10,11,12,13,0"}sspr(sx,sy,w,h,1,1,w,h,not ut.f and u.dir==ut.sdir)pal()if u.alive and ihp>=2then spr(ut.f and 247+f/20,w\3)line(w,unspl"-1,0,-1,8")line(w\ihp,-1,11)end end local function mn(u,res)local wp,x,y=dpath(u,res)if wp then gogth(u,x,y,wp)return res end end local function collect(_ENV,r)if res and res.typ==r then res.qty+=1else res=p("qty=1",r)end if res.qty>=typ.cap then godrop(_ENV,r)end end function drop(_ENV)if res then pres[res.typ]+=res.qty/typ.gr end st.idl,res=11if st.farm then gofarm(_ENV,st.farm)elseif st.y then mn(_ENV,st.y)else rt(_ENV)end end function frm(u)local _ENV=u.st.farm if not farmer then rt(u)elseif gl.cf==0then if ready then fres-=1sproff+=1collect(u,"r")if fres<1then godrop(u)cyc+=1exp,ready=cyc>=typ.mcyc if exp and ai then cyc,exp=0,pay(gl.ren,1,pres)end sproff=exp and(sfx"36"or 32)or 0end u.st.farm=_ENV else fres+=typ.gr sproff,ready=fres*2,fres>=9end end end function atk(u)local typ,e=u.typ,u.st.x if u.upd then local d=dist(e.x-u.x,e.y-u.y)if typ.rng>=d or int(u.r,e.r,0)then u.st.typ=nil if cf%typ.atk_freq==u.id%typ.atk_freq then if e.ap==u.ap then if typ.mnk and e.dmgd then e.hp+=1
if(u.onscr)sfx"20"
else rt(u)end else add(prj,typ.s and p("",typ,u.x-u.dir*typ.px,u.y+typ.py,e.x,e.y,u.ap)or dmg(typ,e))if e.conv>=e.max_hp then if e.qn then e.hp=0else e.pres.pop-=1u.pres.pop+=1e.ply,e.conv=u.ply,0end del(e.sqd,e)sfx"38"end end end else if u.hu and viz[e.k]or typ.los>=d or e.disc then goatk(u,e.k!=u.st.k and e)elseif not e.disc then u.st.typ=nil end if not u.st.typ then rt(u)end end end end function bld(u)if cf%30==0then local _ENV=u.st.x if ct then ct+=1max_hp+=hpr hp+=hpr if ct>=typ.ct then ct=u.hu and sfx"26"reg_bldg(_ENV)if drop then pres.pl+=5elseif farm then gofarm(u,_ENV)end end elseif dmgd and pres.b>=1then hp+=2pres.b-=.1else rt(u)surr(function(t)local _ENV=gl.bldgs[t.k]if _ENV and hu and ct and(u.ant or web)then gobld(u,_ENV)end end,x8,y8,4)end end end function gth(u)local r,x,y=u.st.y,unpack(u.st.p1)local t=mget(x,y)local f=resqty[t]if not f then if u.mnk then pay(pray,-1,res1)elseif not mn(u,r)then godrop(u,r)end elseif cf==u.id then f+=res1.diff*u.ap\33*7local n=g(rtiles,x,y,f)-1collect(u,r)if t<112and(n==f\3or n==f\1.25)then mset(x,y,t+16)elseif n==0then mset(x,y,68)s(dmap_st[r],x,y)s(dmaps[r],x,y,.55)n=qdmaps(r)end s(rtiles,x,y,n)end end function pr(_ENV)local bld=q.typ q.x-=.03333if q.x<=0then if bld.x then local _ENV=bld gl.res1.techs|=tmap x(typ.p1)sfx"33"if up and up<1then up+=1r*=1.75g*=2b*=2t*=1.5done=nil end else local new=gl.unit(bld,x,y,ply),onscr and hu and sfx"19"if bld.ant and rtx and gl.resqty[mget(rtx,rty)]then gogth(new,rtx,rty)else mv(new,rx or x+5,ry or y+5)end end if q.qty>1then q.qty-=1q.x=bld.t else q=nil end end end function w(_ENV)mv(_ENV,x+rs"-6,-5,-4,-3,3,4,5,6",y+rs"-6,-5,-4,-3,3,4,5,6",1)end function cam()local b=btn()
if(b>255)b>>=8
local dx,dy=(b&2)-(b&1)*2,(b&8)/4-(b&4)/2if dget"0"!=2or loser then amx,amy=stat"32",stat"33"else amx+=dx amy+=dy dx,dy=amx\128*2,amy\128*2end cx,cy,amx,amy=mid(cx+dx,256),mid(cy+dy,loser and 128or 151),mid(amx,126),mid(amy,126)mx,my,hbtn=amx+cx,amy+cy mx8,my8=mx\8,my\8end function fsel(func,...)for u in all(sel)do func(u,...)end end function input()cam()foreach(btns,function(b)if int(b.r,{amx,amy,amx,amy},1)then hbtn=b end end)local htile,atkmov,clk=tu(mx8,my8),axn and dget"0">1,lclk or rclk if clk and hbtn then axn=hbtn.fn(rclk)return end if lclk and axn then rclk,axn,llclk=1end if amy>104and not selx then local x,y=amx*20.21-2101.84,amy*21.33-2260.98if mid(x,384)==x and mid(y,256)==y then if rclk and sel1 then sfx"1"fsel(mv,x,y,atkmov)hilite{amx,amy,2,8}elseif not axn and llclk then cx,cy=x-64,y-64cam()end end if clk then to_bld=nil end elseif to_bld then if clk and bldable()then local b=unit(to_bld,mx8*8+to_bld.w\2,my8*8+to_bld.h\2,unspl"1,1,1")fsel(gobld,b)pay(to_bld,1,res1)selx,to_bld=sfx"1",rclk and can_pay(to_bld,res1)and to_bld end elseif btnp"5"and sel1 and sel1.unit and t()-selt<.2then sel,selx={}foreach(units,function(_ENV)add(onscr and hu and idx==gl.sel1.idx and gl.sel,_ENV)end)elseif rclk and sel1 and sel1.hu then if cr()then sfx"0"hilite(hbld)hbld.sproff,hbld.cyc,hbld.exp=0,0pay(ren,1,res1)gofarm(sel1,hbld)elseif can_gth()then sfx"0"hilite(htile)if afarm()then gofarm(sel1,hbld)else fsel(gogth,mx8,my8)end elseif can_bld()then sfx"0"fsel(gobld,hbld)hilite(hbld)elseif can_atk()then sfx"4"fsel(goatk,hunit)hilite(hunit)elseif can_drop()then sfx"0"fsel(godrop,nil,hbld)hilite(hbld)elseif sel1.unit then sfx"1"mvg(sel,mx,my,atkmov,1)hilite(p([[f=.5
c=8]],mx,my))elseif sel1.typ.units then sfx"3"sel1.rx,sel1.ry,sel1.rtx,sel1.rty=mx,my,mx8,my8,resqty[mget(mx8,my8)]and hilite(htile)end elseif not axn then if btnp"5"and not selx then selx,sely,selt=mx,my,t()end if llclk and selx then sb={min(selx,mx),min(sely,my),max(selx,mx),max(sely,my),7}else selx=nil end end end function dpath(u,k)local x,y,dmap,pth,l=u.x8,u.y8,dmaps[k]or{},{},9while l>=.5do local none=1surr(function(t)local w=(dmap[t.k]or 9)+t.d-1if w<l and(u.ai or exp[t.k])then l,x,y,none=w,unpack(t)end end,x,y,1,1)if none then s(dmap,x,y,min(l+1,9))return end add(pth,{x*8+3,y*8+3})end return pth,x,y end function qdmaps(k)dq=split(p[[r=d,b,g,r
g=d,b,r,g
b=d,r,g,b
...
porto=2
portf=13
ptx=]]..split"90,81,81"[dget"0"]),function()axn=not axn return axn end,20,108)end if st.ant or nsel==1then if sel1.ct then draw_port(p[[ptx=72
pty=72
porto=8
portf=9]],function()pay(st,-1,res1)sel1.hp=0end,24,107,sel1.ct/st.ct,5,12)else local q=sel1.q,sel1.farm and ?"ᶜ4⁶jbr⁴i"..sel1.cyc.."/"..st.mcyc.."⁵he⁶:040c1e0d05010706⁵ch⁶:0c1c1014160f0604"
for i,b in next,sel1.prod do if not b.done then draw_port(b,function()if can_pay(b,res1)and(not q or q.typ==b and q.qty<9)then sfx"2"if b.bldg then to_bld=b!=to_bld and b return end prod(sel1,b,1)b.done=b.x else sfx"16"end end,split"88,76,64,52,40,88,76,64"[i],split"106,106,106,106,106,117,117,117"[i],nil,nil,nil,nil,b)end end if q then local b=q.typ draw_port(b,function()b.done=pay(b,-1,res1)if q.qty==1then sel1.q=nil else q.qty-=1end sfx"18"end,b.x and 24or ?"ᶜ7⁶j8r⁴iX"..q.qty
