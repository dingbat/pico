...
techs=0
t=0
npl=0]]aspl"dq,exp,vcache,dmaps,units,restiles,sel,prj,bldgs,nviz,typs,ais,dmap_st"res1,dmap_st.d,posidx,ptr,cf,selt,alert,ban,amx,amy,tot,loser,menu=res.p1,{},split"1,2,3,4",unspl"0x9004,59,0,0,0,64,64,50"for i=2,4do ais[i]=p("boi=0",i)end p[[var=heal
...
in_bld=1
ez_adj=1]],path(u,b.x,b.y),b)end function gogth(u,tx,ty,wp)local t=tile_unit(tx,ty)u.st=p([[t=gth
gth=1
ez_adj=1]],wp or path(u,t.x,t.y),t,f2r[fmget(tx,ty)],tx,ty)end function godrop(u,nxt_res,dropu)local wayp if not dropu then wayp,x,y=dpath(u,"d")dropu=not wayp and units[u.p]end u.st=p([[t=drop
drop=1
in_bld=1
...
function draw_unit(u)local r,fw,w,h,stt,ihp,ux,uy=u.res and u.res.typ or"_",u.fw,u.w,u.h,u.st.typ and"move"or u.st.t,u.max_hp/u.hp,unpack(u.r)local sx,sy,ufps,fr,f,selc=u[stt.."_x"]+resx[r]+u.sproff\8*8,u[stt.."_y"]+resy[r],u[stt.."_fps"],u[stt.."_fr"],u.dead or(cf-u.id)%60,count(sel,u)==1and 9camera(cx-ux,cy-uy)if u.const and u.alive then fillp"23130.5"rect(0,0,w,h,u==sel1 and 9or 12)fillp()local p=u.const/u.typ.const line(fw-1,unspl"0,0,0,5")line(fw*p,0,14)sx+=p\-.5*fw if p<=.15then return end elseif ufps then sx+=f\ufps%fr*fw end pal{selc or u.pres.col,[14]=pal(u.farm and 5,selc or 5)}sspr(sx,sy,w,h,1,1,w,h,not u.fire and u.dir==u.sdir)pal()if u.alive and ihp>=2then if u.fire then spr(247+f/20,w\3)end line(w,unspl"-1,0,-1,8")line(w\ihp,-1,11)end end function drop(u)if u.res then u.pres[u.res.typ]+=u.res.qty/u.typ.gr end u.st.idl,u.res=11if u.st.farm then gofarm(u,u.st.farm)elseif u.st.y then mine_nxt(u,u.st.y)else rest(u)end end function frm(u)local _ENV,g=u.st.farm,_ENV if not farmer then g.rest(u)elseif g.cf==0then if ready then fres-=1sproff+=1g.collect(u,"r")if fres<1then g.godrop(u)cyc+=1exp,ready=cyc>=typ.mcyc if exp and ai then cyc,exp=0,g.pay(g.renew,1,pres)end sproff=exp and(g.sfx"36"or 32)or 0end u.st.farm=_ENV else fres+=typ.gr sproff,ready=fres*2,fres>=9end end end function atk(u)local typ,e=u.typ,u.st.x if u.upd then local d=dist(e.x-u.x,e.y-u.y)if typ.range>=d or int(u.r,e.r,0)then u.st.typ=nil if cf%typ.atk_freq==u.id%typ.atk_freq then if e.ap==u.ap then if typ.monk and e.dmgd then e.hp+=1
if(u.onscr)sfx"20"
else rest(u)end else add(prj,typ.prj_s and p("",typ,u.x-u.dir*typ.prj_xo,u.y+typ.prj_yo,e.x,e.y,u.ap)or dmg(typ,e))if e.conv>=e.max_hp then if e.queen then e.hp=0else e.pres.p-=1u.pres.p+=1e.p,e.conv=u.p,0end del(e.sqd,e)sfx"38"end end end else if u.hu and viz[e.k]or typ.los>=d then goatk(u,e.k!=u.st.k and e)elseif not e.disc then u.st.typ=nil end if not u.st.typ then rest(u)end end end end function bld(u)if cf%30==0then local _ENV,g=u.st.x,_ENV if const then const+=1max_hp+=hpr hp+=hpr if const>=typ.const then const=u.hu and g.sfx"26"g.reg_bldg(_ENV)if drop then pres.pl+=5elseif farm then g.gofarm(u,_ENV)end end elseif dmgd and pres.b>=1then hp+=2pres.b-=.1else g.rest(u)g.surr(function(t)local _ENV=g.bldgs[t.k]if _ENV and hu and const and(u.ant or web)then g.gobld(u,_ENV)end end,x8,y8,4)end end end function gth(u)local r,x,y=u.st.y,unpack(u.st.p1)local t=mget(x,y)local f=resqty[t]if not f then if u.monk then pay(pray,-1,res1)elseif not mine_nxt(u,r)then godrop(u,r)end elseif cf==u.id then f+=res1.diff*u.ap\33*10local n=g(restiles,x,y,f)collect(u,r)if t<112and(n==f\3or n==f\1.25)then mset(x,y,t+16)elseif n==1then mset(x,y,68)s(dmap_st[r],x,y)s(dmaps[r],x,y,.55)qdmaps(r)end s(restiles,x,y,n-1)end end function produce(u)local _ENV,gl=u,_ENV local bld=q.typ q.x-=.03333if q.x<=0then if bld.x then local _ENV=bld gl.res1.techs|=tmap x(typ.p1)gl.sfx"33"if up and up<1then up+=1r*=1.75g*=2b*=2t*=1.5done=nil end else local new=gl.unit(bld,x,y,p),onscr and hu and gl.sfx"19"if bld.ant and rtx and gl.resqty[mget(rtx,rty)]then gl.gogth(new,rtx,rty)else gl.move(new,rx or x+5,ry or y+5)end end if q.qty>1then q.qty-=1q.x=bld.t else q=nil end end end function mine_nxt(u,res)local wp,x,y=dpath(u,res)if wp then gogth(u,x,y,wp)return res end end 
-->8
--utils
...
function comp(f,g)return function(...)return f(g(...))end end function a(v,...)_ENV[v]={},...and a(...)end pspl,rndspl,unspl,campal,fmget=comp(pal,split),comp(rnd,split),comp(unpack,split),comp(camera,pal),comp(fget,mget)unl,unspr,aspl,typs,stp,pcol,hlt,diff,menu,loser,cx,cy,cvx,cvy=comp(line,unspl),comp(spr,unspl),comp(a,unspl),{},split"-9:-20,263:-20,263:148,-9:148",split"1,2,0,3,1,0,2,1,3,0",unspl"-10,0,63,0,0,30,1,1"p[[var=f2r
7=r
10=g
11=g
19=b
...
88788788ff88188f888f8fdff88188ffff3b33dfffbbfbffff44454ff4494fffffff67cccccccccccc76ffff1111cccccccc1111fffff7ffffffffffffffffff
