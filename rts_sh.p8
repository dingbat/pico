...
techs=0
t=0
npl=0]]aspl"dq,exp,vc,dmaps,units,restiles,sel,prj,bldgs,nviz,ais,dmap_st"res1,dmap_st.d,pi,ptr,cf,selt,alert,ban,amx,amy,tot,loser,menu=res.p1,{},split"1,2,3,4",unspl"0x9004,59,0,0,0,64,64,50"for i=2,4do ais[i]=p("boi=0",i)end p[[var=heal
qty=.00083]]p[[var=ren
r=0
g=0
b=6
...
g=d,b,r,g
b=d,r,g,b
d=b,g,r,d]][r])end function dmap()if cdq then for i=1,#cdq.typ do if i>20then return end local pt=deli(cdq.typ)cdq.p1[pt.k]=cdq.c if cdq.c<8then surr(function(t)cdq.p3[t.k]=cdq.p3[t.k]or add(cdq.p2,t)end,unpack(pt))end end cdq.c+=1cdq.typ,cdq.p2=cdq.p2,{}if cdq.c==9then dmaps[cdq.x],cdq=cdq.p1 end else local q,o=deli(dq),{}if q then if not dmap_st[q]then dmap_st[q]={}for x=0,48do for y=0,32do if f2r[fm(x,y)]==q then s(dmap_st[q],x,y,{x,y})end end end end for i,t in next,dmap_st[q]do if surr(nil,unpack(t))then add(o,t).k=i end end cdq=p("c=0",o,q)end end end function nr(gx,gy)for n=0,16do local best_d,best_t=32767surr(function(t)local d=dist(t[1]*8+4-gx,t[2]*8+4-gy)if d<best_d then best_t,best_d=t,d end end,gx\8,gy\8,n)
...
porto=2
portf=13
ptx=]]..split"90,81,81"[dget"0"]),function()axn=not axn end,20,108)end if st.ant or nsel==1then if sel1.const then draw_port(p[[ptx=72
pty=72
porto=8
